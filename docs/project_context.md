...
- **Poppins and IBM Plex Sans are Latin-only — they have no Arabic glyphs.** `AppText` (`src/components/ui/app-text.tsx`) only applies these custom font families when `language !== "ar"`; Arabic text falls back to the platform default font, which has full Arabic coverage. On native, an unsupported glyph in an explicitly-set font doesn't reliably fall back to a system font the way browsers do on web — it can render blank. Don't force `themeFontFamily`/`font-sans` onto text that might be Arabic.
- **Absolutely-positioned overlay elements inside a `SafeAreaView` should use `useSafeAreaInsets()` directly**, not just a `top-4`-style class relying on the parent's padding. `LanguageToggle` (`src/components/ui/language-toggle.tsx`) positions itself at `insets.top + 12` for this reason — more robust across devices/cutouts than trusting `SafeAreaView`'s padding alone to be measured correctly by the time the child renders.
