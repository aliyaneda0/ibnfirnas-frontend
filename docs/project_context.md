...
- **Design system**: `src/components/ui/` — `AppText`, `PrimaryButton`, `TextField`, `Screen`, `LanguageToggle`, `BrandWordmark`, `AnimatedSplash`, `AppHeader`, `HeroCarousel`, `WhatsAppFab`, `Skeleton`/`SkeletonCard`, `ErrorState`/`EmptyState`, `Badge`. Tokens in `src/config/design-tokens.js` (colors + font families), consumed by both `tailwind.config.js` (build-time) and app code (`@/config/design-tokens`).
